                ...
                char *enc_buf, size_t enc_buf_len)
{
    ...
    buf_len = (uint32_t)BIO_ctrl_pending(r_prikey);
    if (buf_len == 0) {
        goto end;
    }
    ...
    if ((size_t)BIO_read(r_prikey, buf, (int)buf_len) != buf_len) {
        goto end;
    }
    sealed_data_len = sgx_calc_sealed_data_size(strlen((const char *)ADD_DATA_RAW), buf_len);
    if (sealed_data_len == UINT32_MAX || enc_buf_len < sealed_data_len) {
        goto end;
    }
    temp_buf = (uint8_t *)malloc(sealed_data_len);
    if (temp_buf == NULL) {
        goto end;
    }
    retval = sgx_seal_data(strlen((const char*)ADD_DATA_RAW), (uint8_t*)ADD_DATA_RAW, buf_len, (uint8_t *)buf, sealed_data_len, (sgx_sealed_data_t *)temp_buf);
    if (retval != SGX_SUCCESS) {
        goto end;
    }
    memcpy(enc_buf, temp_buf, sealed_data_len);
    res = sealed_data_len;
...
int unseal_enc_data(char **data_p, size_t *data_len_p, const char *enc_data)
{
    ...
    add_len = sgx_get_add_mac_txt_len((const sgx_sealed_data_t *)enc_data);
    data_len = sgx_get_encrypt_txt_len((const sgx_sealed_data_t *)enc_data);
    ...
    retval = sgx_unseal_data((const sgx_sealed_data_t *)enc_data, (uint8_t *)add_data, (uint32_t *)&add_len, (uint8_t *)data, (uint32_t *)&data_len);
    if (retval != SGX_SUCCESS) {
        goto end;
    }
...
int set_ctx_key(SSL_CTX *ctx, const char *enc_key_file_name)
{
    ...
    in_bio = BIO_new_mem_buf(raw_key, (int)raw_key_len);
    if (in_bio == NULL) {
        goto end;
    }
